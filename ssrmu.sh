...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget https://content.evernote.com/shard/s397/sh/94b179cf-e80b-4e63-b8a6-8a579bf0a6d5/415a0d64878c61b6/res/fdcbe8db-3301-43bc-aa04-8653de6edd9a/libsodium-1.0.18.tar.gz
		echo -e "${Info} 解压..."
		tar -xf libsodium-1.0.18.tar.gz && cd libsodium-1.0.18-RELEASE
		echo -e "${Info} 编译安装..."
		./configure --disable-maintainer-mode && make -j2 && make install
	fi
	ldconfig
